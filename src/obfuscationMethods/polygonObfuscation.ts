...
const getDividedPolygonElements = (width: number, height: number, fill: string, divisionDepth: number, changeToPaths: boolean) => {
...
const getDividedIntoPaths = (width: number, height: number, xInit: number, yInit: number, depth: number, paths: SVGElement[], fill: string) => {
  if (depth > 1) {
    ...
    createCompletedPath(fill, point1, point2, point3).forEach((p) => paths.push(p));
...
const createCompletedPath = (fill: string, point1: Point, point2: Point, point3: Point) => {
  ...
  return [createPartialPath(fill, ceilTo1(opacity), point1, point2, point3), createPartialPath(fill, ceilTo1(leftOpacity), point1, point2, point3)];
};
